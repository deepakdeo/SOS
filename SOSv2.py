...
# Start: Mar 4, 2016, Last update: Apl 24, 2020
...
cfgfile = 'ska_mid197_new.cfg'   # telescope configuration files are in your casa-release-..../data/alma/simmos/
...
dT = 2.0	       # each scan duration in seconds
...
def sclimg(imageref,fratio,fvalue,fvalue2):
	...
	#imhead(imagename=imagein, mode='put', hdkey='crval3', hdvalue=str(fvalue2) + 'GHz')  # setting the centralfreq, for SKA downloaded image, else above line
...
def visibility(cfgfile,myvis,imageref,alpha,chan,delta_f,int_time,RT,nscans,ST,dT,sg):
	...
	telescope=col[5]    # name of observatory
	...
	#frequency = imhead(imagename=imageref, mode='get', hdkey='crval3') # for SKA downloaded image, else above
